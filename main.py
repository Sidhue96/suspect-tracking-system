...
if __name__ == '__main__':
	...
	for f in os.listdir(path):
		...
		if os.path.exists("suspect_model.yaml"):
			recognizer.load("suspect_model.yaml")
			for video in list_of_videos:
				...
				confidence = count_suspect/100
				if confidence > 0.1:
					subject_one = 0
					subject_two = 0
					for i in range(0,len(suspect_lab)):
						if suspect_lab[i] == 1:
							subject_one = subject_one+1
						if suspect_lab[i] == 2:
							subject_two = subject_two+1
					if (subject_one / len(suspect_lab)) > 0.35:
						db.suspect.insert_one({"suspect":"1","camera":video[-5]})
					if (subject_two / len(suspect_lab)) > 0.35:
						db.suspect.insert_one({"suspect":"2","camera":video[-5]})
					print('Suspect Found in Video: '+video[-5])
					print('With Confidence: '+str(confidence))
				else:
					print('Suspect not found.. Continuing next video')
				camera.release()
